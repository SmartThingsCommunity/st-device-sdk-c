...
* [Typedefs](#typedefs)
  ...
  * [st_device_status](#st_device_status)
  * [iot_pin_t](#iot_pin_t)
...
There are cmake configs you can include to use some SDK features or specify porting system.

For more detail information, you can refer [config reference](./STDK_Config.md).
...
This function tries to connect server. You can receive status transition events via `status_cb`. For more information about device status, refer [device state diagram](./Device_State_Diagram.md).
...
If there is no error, this function returns a request id which is a unique value identifying this publish request. The same value is reported back through the `IOT_NOTI_TYPE_SEND_SUCCESS` / `IOT_NOTI_TYPE_SEND_FAILED` notification, so the caller can match a send request with its asynchronous result. The request id is generated by the MQTT layer and is not a monotonically increasing sequence number.  
...
__Returns__  
return "request id" (which is positive integer) if successful, negative integer for error case.
...
```c
#define ST_CAP_SEND_ATTR_STRINGS_ARRAY(cap_handle, attribute, value_string_array, array_num, unit, data, output_request_id)\
{\
	IOT_EVENT *attr = NULL;\
	iot_cap_val_t value;\
\
...
typedef union {
	...
	/* send fail case */
	struct _send_fail {
		int failed_request_id;		                /**< request id of the failed send (value returned by st_cap_send_attr). */
	} send_fail;
	/* commands */
	struct _commands {
