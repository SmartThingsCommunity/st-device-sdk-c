...
iot_error_t iot_nv_init(unsigned char *device_info, size_t device_info_len)
{
    ...
    if (data == NULL) {
        IOT_ERROR("failed to allocate memory for nv data");
...
iot_error_t iot_nv_get_wifi_prov_data(struct iot_wifi_prov_data *wifi_prov)
{
    ...
    if (ret == IOT_ERROR_NONE) {
        if (read_len >= IOT_WIFI_PROV_MAC_STR_LEN) {
            ...
            if (ret != IOT_ERROR_NONE) {
                IOT_INFO("Saved AP BSSID is invalid string(%d):%s", ret, wifi_prov->mac_str);
            ...
            ret = iot_util_convert_mac_str(&wifi_prov->bssid, wifi_prov->mac_str, sizeof(wifi_prov->mac_str));
            if (ret != IOT_ERROR_NONE) {
...
iot_error_t _iot_nv_get_certificate_serial_number(char **cert_sn)
{
    ...
    ret = iot_security_base64_decode((const unsigned char *)trimmed_cert, trimmed_cert_len, (unsigned char *)cert_der,
                                     trimmed_cert_len, &cert_der_len);
    if (ret) {
