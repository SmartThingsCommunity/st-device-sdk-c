	...
	switch (cmd->cmd_type) {
		...
		case IOT_COMMAND_CLOUD_CONNECTING:
			...
			if (err == IOT_ERROR_MQTT_REJECT_CONNECT ||	err == IOT_ERROR_INVALID_ARGS) {
				...
				ctx->next_connection_retry_timer = iot_os_timer_create(_next_connection_retry_timeout, next_retry_time, ctx);
				if (!ctx->next_connection_retry_timer) {
		...
		case IOT_COMMAND_NOTIFICATION_RECEIVED:
			...
			} else if (noti->type == (iot_noti_type_t)_IOT_NOTI_TYPE_DEV_ONBOARDED) {
				IOT_INFO("Device is onboarded.");
				if (ctx->noti_cb)
					ctx->noti_cb(noti, ctx->noti_usr_data);
			}
...
static void _device_work_queue_task(void *parm)
{
	...
	for( ; ;) {
		curr_events = iot_os_eventgroup_wait_bits(ctx->work_queue_signal,
			DEVICE_WORK_QUEUE_TASK_SIGNAL_ALL, true, IOT_OS_WAIT_FOREVER);
...
iot_error_t iot_put_device_work(struct iot_context *ctx, device_work_handler handler,
		device_work_param param)
{
	device_work_data_t work;
	iot_error_t err;
