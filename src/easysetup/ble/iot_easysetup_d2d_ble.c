...
#define COMBO_SN_LEN 16
#define COMBO_PUB_KEY_LEN 8
#define COMBO_PLAIN_SN_LEN 8
...
char *_es_json_parse_string(JSON_H *json, const char *name)
{
    ...
    IOT_DEBUG("'%s' (%d): %s", name, buf_len, recv->valuestring);
...
iot_error_t _es_time_set(unsigned char *time)
{
    ...
    if (sscanf((char *)time, "%4d-%2d-%2dT%2d.%2d.%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min,
               &tm.tm_sec) != 6) {
...
STATIC_FUNCTION
iot_error_t iot_easysetup_get_pubkey(iot_security_context_t **state, unsigned char **pubkey, size_t *pubkey_len)
{
    iot_error_t err;
    ...
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("Fail to init ecdh (%d)", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_SECURITY_ECDH_INIT, err);
        err = IOT_ERROR_EASYSETUP_BLE_ECDH_SETUP_FAIL;
        goto out;
    };

    iot_security_ecdh_params_t ecdh_params = {0};

    ecdh_params.key_id = IOT_SECURITY_KEY_ID_DEVICE_PUBLIC;

    err = iot_security_manager_get_key(context, ecdh_params.key_id, &own_pubkey_buf);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("Fail to get ecdh pubkey 0x%x", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_PUBLIC_KEY_GET_FAIL, err);
        err = IOT_ERROR_EASYSETUP_PUBLIC_KEY_GET_FAIL;
        goto out;
    }

    if ((*pubkey = iot_os_malloc(own_pubkey_buf.len)) == NULL) {
        IOT_ERROR("failed to malloc for buf");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_MEM_ALLOC_ERROR, err);
        err = IOT_ERROR_MEM_ALLOC;
        goto out;
    }
    memcpy(*pubkey, own_pubkey_buf.p, own_pubkey_buf.len);
out:
    return err;
}
...
iot_error_t _es_deviceinfo_handler(struct iot_context *ctx, char *in_payload, char **out_payload)
{
    ...
    err = iot_easysetup_ble_ecdh_compute_shared_signature(&ctx->easysetup_security_context, sec_random, &dev_cert,
                                                          &sub_cert, &spub_key, &orin_spub_key_len, &signature,
                                                          &orin_signature_len);
    if (err != IOT_ERROR_NONE) {
    ...
    err = iot_security_base64_encode_urlsafe(spub_key, orin_spub_key_len, spub_key_str, sizeof(spub_key_str),
                                             &spub_key_len);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("base64url encode error 0x%x", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_ENCODE_ERROR, err);
    ...
    err = iot_security_base64_encode_urlsafe(signature, orin_signature_len, signature_str, sizeof(signature_str),
                                             &signature_len);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("base64url encode error 0x%x", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_ENCODE_ERROR, err);
...
iot_error_t _es_deviceinfo_handler(struct iot_context *ctx, char **out_payload)
{
    ...
    size_t pubkey_encode_buf_len = 0;
    unsigned char *encode_buf = NULL;
    unsigned char pubkey_encode_buf[IOT_SECURITY_B64_ENCODE_LEN(IOT_SECURITY_SHA256_LEN)] = {
        0,
    };
    ...
    err = iot_easysetup_get_pubkey(&ctx->easysetup_security_context, &pubkey, &pubkey_len);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("get public key fail %d", err);
        goto exit;
    }
    ...
    if ((combo_sn = iot_os_malloc(COMBO_SN_LEN)) == NULL) {
        IOT_ERROR("failed to malloc for combo sn");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_MEM_ALLOC_ERROR, 0);
        err = IOT_ERROR_MEM_ALLOC;
        goto exit;
    }

    memcpy(combo_sn, pubkey, COMBO_PUB_KEY_LEN);
    memcpy(combo_sn + COMBO_PUB_KEY_LEN, dev_sn + devsn_len - COMBO_PLAIN_SN_LEN, COMBO_PLAIN_SN_LEN);

    hash_buf.len = IOT_SECURITY_SHA256_LEN;
    hash_buf.p = (unsigned char *)iot_os_malloc(hash_buf.len);

    err = iot_security_sha256(combo_sn, COMBO_SN_LEN, hash_buf.p, hash_buf.len);
    if (err != IOT_ERROR_NONE) {
    ...
    err = iot_security_base64_encode_urlsafe(hash_buf.p, hash_buf.len, pubkey_encode_buf, sizeof(pubkey_encode_buf),
                                             &pubkey_encode_buf_len);
    if (err != IOT_ERROR_NONE) {
    ...
    if (ctx->devconf.combo_sn) {
        iot_os_free(ctx->devconf.combo_sn);
        ctx->devconf.combo_sn = NULL;
    }

    ctx->devconf.combo_sn = iot_os_malloc(pubkey_encode_buf_len + 1);
    if (!ctx->devconf.combo_sn) {
        IOT_ERROR("failed to malloc for hashed combo sn");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_MEM_ALLOC_ERROR, 0);
        err = IOT_ERROR_MEM_ALLOC;
        goto exit;
    }

    memset(ctx->devconf.combo_sn, '\0', pubkey_encode_buf_len + 1);
    memcpy(ctx->devconf.combo_sn, pubkey_encode_buf, pubkey_encode_buf_len);
    ...
    JSON_ADD_ITEM_TO_OBJECT(data, "protocolVersion", JSON_CREATE_STRING(STDK_D2D_PROTOCOL_VERSION));
    JSON_ADD_ITEM_TO_OBJECT(data, "firmwareVersion", JSON_CREATE_STRING(ctx->device_info.firmware_version));
    JSON_ADD_ITEM_TO_OBJECT(data, "hashedSn", JSON_CREATE_STRING(ctx->devconf.combo_sn));
    JSON_ADD_NUMBER_TO_OBJECT(data, "wifiSupportFrequency", (double)iot_bsp_wifi_get_freq());
    ...
    err = iot_security_base64_encode_urlsafe(iv_buf.p, iv_buf.len, encode_buf, encode_buf_len, &base64_written);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("base64 encode error!!");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_ENCODE_ERROR, 0);
        err = IOT_ERROR_EASYSETUP_BASE64_ENCODE_ERROR;
...
iot_error_t _es_keyinfo_handler(struct iot_context *ctx, char *in_payload, char **out_payload)
{
    ...
    err = iot_security_base64_decode_urlsafe((unsigned char *)p_cpubkey_str, input_len, decode_buf, output_len,
                                             &result_len);
    if (err) {
    ...
    err = iot_security_base64_decode_urlsafe((unsigned char *)p_datetime_str, input_len, decode_buf, output_len,
                                             &result_len);
    if (err) {
        IOT_ERROR("base64 decode error!! : %d", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_DECODE_ERROR, err);
    ...
    err = iot_security_base64_decode_urlsafe((unsigned char *)p_regionaldatetime_str, input_len, decode_buf, output_len,
                                             &result_len);
    if (err) {
        IOT_ERROR("base64 decode error!! : %d", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_DECODE_ERROR, err);
    ...
    err = iot_security_base64_decode_urlsafe((unsigned char *)p_timezoneid_str, input_len, decode_buf, output_len,
                                             &result_len);
    if (err) {
        IOT_ERROR("base64 decode error!! : %d", err);
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_BASE64_DECODE_ERROR, err);
...
iot_error_t _es_keyinfo_handler(struct iot_context *ctx, char *in_payload, char **out_payload)
{
    ...
    err = iot_security_base64_decode_urlsafe(p_spub_str, strlen((char *)p_spub_str), key_spub_sign,
                                             sizeof(key_spub_sign), &spub_len);
    if (err) {
    ...
    err = iot_security_base64_decode(p_rand_str, strlen((char *)p_rand_str), (unsigned char *)rand_asc,
                                     sizeof(rand_asc), &rand_asc_len);
    if (err) {
...
STATIC_FUNCTION
iot_error_t _es_confirm_check_manager(struct iot_context *ctx, enum ownership_validation_feature confirm_feature,
                                      char *sn)
{
    char *dev_sn = NULL;
    ...
    size_t devsn_len;
    iot_error_t err = IOT_ERROR_NONE;
    unsigned char hash_buffer[IOT_SECURITY_SHA512_LEN] = {
        0,
    };
    char hash_string_buffer[IOT_SECURITY_SHA512_LEN * 2 + 1];
    int i;

    iot_os_eventgroup_clear_bits(ctx->iot_events,
                                 IOT_EVENT_BIT_EASYSETUP_CONFIRM | IOT_EVENT_BIT_EASYSETUP_CONFIRM_DENY);
    ctx->curr_otm_feature = confirm_feature;

    IOT_REMARK("IOT_STATE_PROV_CONFIRMING");

    err = iot_state_update(ctx, IOT_STATE_PROV_CONFIRM, IOT_STATE_OPT_NEED_INTERACT);
    if (err != IOT_ERROR_NONE) {
    ...
    switch (confirm_feature) {
        ...
        case OVF_BIT_BUTTON:
            ...
            curr_event = iot_os_eventgroup_wait_bits(
                ctx->iot_events, IOT_EVENT_BIT_EASYSETUP_CONFIRM | IOT_EVENT_BIT_EASYSETUP_CONFIRM_DENY, false,
...
{
    char *output_ptr = NULL;
    char wifi_bssid[WIFIINFO_BUFFER_SIZE] = {
        0,
    };
    char ssid[IOT_WIFI_PROV_SSID_STR_LEN + 1] = {
        0,
    };
    JSON_H *root = NULL;
    JSON_H *array = NULL;
            ...
            if ((data = JSON_GET_OBJECT_ITEM(root, "data")) == NULL) {
                ...
                if ((wifi_credential = JSON_GET_OBJECT_ITEM(data, "mobileWifiCredential")) == NULL) {
                    ...
                    if ((item = JSON_GET_OBJECT_ITEM(wifi_credential, "ssid")) == NULL) {
                        ...
                        IOT_SENSITIVE_INFO_SSID("ssid :", ssid);
...
STATIC_FUNCTION
iot_wifi_auth_mode_t _decide_wifi_auth_mode(const JSON_H *item, struct iot_wifi_prov_data *wifi_prov,
                                            const struct iot_context *ctx)
{
...
iot_error_t _es_wifi_prov_parse(struct iot_context *ctx, char *in_payload)
{
    ...
    if ((item = JSON_GET_OBJECT_ITEM(wifi_credential, "macAddress")) == NULL) {
        ...
        IOT_SENSITIVE_INFO_MAC("mac addr:", wifi_prov->bssid.addr);
    }
    ...
    IOT_SENSITIVE_INFO_SSID("ssid:", wifi_prov->ssid);
    IOT_SENSITIVE_INFO("security_type: %d", wifi_prov->security_type);
...
iot_error_t _es_wifiprovisioninginfo_handler(struct iot_context *ctx, char *in_payload, char **out_payload)
{
    ...
    if (ctx->lookup_id == NULL) {
        ...
        err = iot_get_random_id_str(ctx->lookup_id, (IOT_REG_UUID_STR_LEN + 1));
        if (err != IOT_ERROR_NONE) {
...
#else
    err = st_create_log_dump((IOT_CTX *)ctx, &log_dump, log_dump_size, &written_size,
...
#if defined(CONFIG_STDK_IOT_CORE_EASYSETUP_OFFLINE_DIAGNOSTICS)
static iot_error_t _es_offline_diagnostics_connection_info_handler(struct iot_context *ctx, char **out_payload)
{
    char *output_ptr = NULL;
    JSON_H *root = NULL;
    JSON_H *data = NULL;
    JSON_H *con_info = NULL;
    iot_error_t err = IOT_ERROR_NONE;

    root = JSON_CREATE_OBJECT();
    if (!root) {
        IOT_ERROR("Root JSON Object create failed");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_JSON_CREATE_ERROR, 0);
        err = IOT_ERROR_EASYSETUP_JSON_CREATE_ERROR;
        goto out;
    }
    ...
    data = JSON_CREATE_OBJECT();
    if (!data) {
        IOT_ERROR("Data JSON Object create failed");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_JSON_CREATE_ERROR, 0);
        err = IOT_ERROR_EASYSETUP_JSON_CREATE_ERROR;
        goto out;
    }
    ...
    con_info = JSON_CREATE_OBJECT();
    if (!con_info) {
        IOT_ERROR("info JSON Object create failed");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_JSON_CREATE_ERROR, 0);
        err = IOT_ERROR_EASYSETUP_JSON_CREATE_ERROR;
        goto out;
    }
    JSON_ADD_ITEM_TO_OBJECT(data, "connectionInfo", con_info);

    JSON_ADD_STRING_TO_OBJECT(con_info, "mnmo", iot_bsp_get_bsp_name());
    JSON_ADD_STRING_TO_OBJECT(con_info, "network_type", "wireless");
    if (iot_bsp_wifi_get_status() == IOT_ERROR_NONE) {
        ...
        JSON_ADD_STRING_TO_OBJECT(con_info, "network_status", "connected");
        ret = iot_bsp_wifi_get_mac(&mac);
        if (ret == IOT_ERROR_NONE) {
            snprintf(wifi_bssid, sizeof(wifi_bssid), "%02X:%02X:%02X:%02X:%02X:%02X", mac.addr[0], mac.addr[1],
                     mac.addr[2], mac.addr[3], mac.addr[4], mac.addr[5]);
            JSON_ADD_STRING_TO_OBJECT(con_info, "ap_mac", wifi_bssid);
        }
        JSON_ADD_NUMBER_TO_OBJECT(con_info, "ap_freq", (double)iot_bsp_wifi_get_freq());
    } else {
        JSON_ADD_STRING_TO_OBJECT(con_info, "network_status", "disconnected");
    }
    if (ctx->last_st_ecode.ecode_type == IOT_ST_ECODE_NONE) {
        JSON_ADD_STRING_TO_OBJECT(con_info, "online_status", "online");
    } else {
        JSON_ADD_STRING_TO_OBJECT(con_info, "online_status", "offline");
    }
    JSON_ADD_STRING_TO_OBJECT(con_info, "last_error_code", ctx->last_st_ecode.ecode);

    JSON_ADD_STRING_TO_OBJECT(data, "version", "1.0");
    JSON_ADD_NUMBER_TO_OBJECT(root, "errorcode", 0);

    output_ptr = JSON_PRINT(root);
    IOT_INFO("connection_info result : %s", output_ptr);
    *out_payload = output_ptr;
out:
    if (root)
        JSON_DELETE(root);
...
void offline_diagnostics_wifi_update_command_handler(struct iot_context *ctx, device_work_param param)
{
    char *output_ptr = NULL;
    JSON_H *root = NULL;
    iot_error_t err = IOT_ERROR_NONE;
    ...
    if (ctx->offline_diagnostics_wifiupdate_timeout) {
        ...
        ctx->offline_diagnostics_wifiupdate_timeout = NULL;
    }
    ...
    JSON_ADD_NUMBER_TO_OBJECT(root, "errorcode", (double)-(err));
    output_ptr = JSON_PRINT(root);

    iot_easysetup_ble_send_response(IOT_EASYSETUP_BLE_STEP_OFFLINE_DIAGNOSTICS_RECOVERY, output_ptr,
                                    strlen(output_ptr));
out:
...
static void _offline_diagnostics_wifi_update_timeout(iot_os_timer_handle handle, void *user_data)
{
    struct iot_context *ctx = (struct iot_context *)user_data;
    iot_error_t err = IOT_ERROR_NONE;
    IOT_INFO("Timeout");

    err = iot_put_device_work(ctx, offline_diagnostics_wifi_update_command_handler, NULL);
    if (err != IOT_ERROR_NONE) {
        IOT_ERROR("Failed to put offline diagnostic wifi update command work");
    }
    iot_os_timer_delete(ctx->offline_diagnostics_wifiupdate_timeout);
    ctx->offline_diagnostics_wifiupdate_timeout = NULL;
}

static iot_error_t _es_offline_diagnostics_recovery_handler(struct iot_context *ctx, char *in_payload,
                                                            char **out_payload)
{
    char *output_ptr = NULL;
    JSON_H *root = NULL;
    ...
    if ((recv = JSON_GET_OBJECT_ITEM(data, "command")) == NULL) {
        IOT_ERROR("no command info");
        IOT_ES_DUMP(IOT_DEBUG_LEVEL_ERROR, IOT_DUMP_EASYSETUP_INVALID_REQUEST, 0);
        err = IOT_ERROR_EASYSETUP_INVALID_REQUEST;
        goto out;
    }

    if (!strcmp(recv->valuestring, "restartThing")) {
        IOT_INFO("Reboot!!!");
        err = iot_command_send(ctx, IOT_COMMAND_SYSTEM_REBOOT, NULL, 0);
        if (err) {
            IOT_ERROR("failed to system reboot");
            goto out;
        }
    } else if (!strcmp(recv->valuestring, "updateWifi")) {
        err = _es_wifi_prov_parse(ctx, in_payload);
        if (err) {
            IOT_ERROR("failed to parse wifi_prov");
            goto out;
        }
        ...
        ctx->offline_diagnostics_wifiupdate_timeout =
            iot_os_timer_create(_offline_diagnostics_wifi_update_timeout, 60000, ctx);
        if (!ctx->offline_diagnostics_wifiupdate_timeout) {
            IOT_ERROR("failed to malloc for offline diagnostics wifi updatea timer");
            err = IOT_ERROR_EASYSETUP_INTERNAL_SERVER_ERROR;
            goto out;
        } else {
            iot_os_timer_start(ctx->offline_diagnostics_wifiupdate_timeout);
        }

        err = IOT_ERROR_EASYSETUP_REQUEST_PENDING;
        goto out;
    } else {
        IOT_ERROR("not supported command");
        err = IOT_ERROR_EASYSETUP_INVALID_REQUEST;
        goto out;
    }
...
static iot_error_t _es_offline_diagnostics_connection_info_handler(struct iot_context *ctx, char **out_payload)
{
    return IOT_ERROR_EASYSETUP_NOT_SUPPORTED;
}
...
void offline_diagnostics_wifi_update_command_handler(struct iot_context *ctx, device_work_param param)
{
    IOT_ERROR("Offline diagnostics feature not enabled");
}
#endif
