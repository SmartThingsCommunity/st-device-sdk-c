...
iot_error_t iot_easysetup_create_ssid(struct iot_devconf_prov_data *devconf, char *ssid, size_t ssid_len)
{
    ...
    unsigned char last_sn[HASH_SIZE + 1] = {
        0,
    };
    unsigned char hashed_sn[HASH_SIZE + 1] = {
        0,
    };
    size_t length;
    int i, setup_type;
    ...
    last_sn[HASH_SIZE] = '\0';
    IOT_INFO(">> %s[%c%c%c%c] <<", devconf->device_onboarding_id, last_sn[0], last_sn[1], last_sn[2], last_sn[3]);

    setup_type = SETUP_TYPE_HYBRID_SERIAL_NUMBER;

    if (devconf->ssid_version == 4) {
        snprintf(ssid_build, sizeof(ssid_build), "%s_E4%4s%3s%1d%4s%4s", devconf->device_onboarding_id, devconf->mnid,
                 devconf->setupid, setup_type, hashed_sn, last_sn);
    } else if (devconf->ssid_version == 5) {
        snprintf(ssid_build, sizeof(ssid_build), "%s_E5%4s%3s%4s%4s", devconf->device_onboarding_id, devconf->mnid,
                 devconf->setupid, hashed_sn, last_sn);
    } else {
