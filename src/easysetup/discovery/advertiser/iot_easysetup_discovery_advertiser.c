...
static void generate_hybrid_serial(struct iot_context *ctx, uint8_t *hybrid_serial_out)
{
    ...
    unsigned char hash_buffer[IOT_SECURITY_SHA256_LEN] = {
        0,
    };
    unsigned char base64url_buffer[IOT_SECURITY_B64_ENCODE_LEN(IOT_SECURITY_SHA256_LEN)] = {
        0,
    };
    char hybrid_serial[HYBRID_SERIAL_NUMBER_SIZE + 1] = {
        0,
    };
    ...
    err = iot_security_base64_encode_urlsafe(hash_buffer, sizeof(hash_buffer), base64url_buffer,
                                             sizeof(base64url_buffer), &base64_written);
    if (err != IOT_ERROR_NONE) {
...
static void print_adv_data(adv_data *data)
{
    ...
    IOT_INFO(
        "MN data : [ Control: 0x%02X, ServiceId : 0x%02X, PacketVersion : 0x%02X, "
        ...
        data->mn_data[0], data->mn_data[1], data->mn_data[2], data->mn_data[3], data->mn_data[4], data->mn_data[5],
        data->mn_data[6], data->mn_data[7], data->mn_data[8], data->mn_data[9], data->mn_data[10], data->mn_data[11],
        data->mn_data[12], data->mn_data[13], data->mn_data[20], data->mn_data[21], data->mn_data[22],
        data->mn_data[23], data->mn_data[24], data->mn_data[25], data->mn_data[26], data->mn_data[27],
        data->mn_data[28], data->mn_data[29], data->mn_data[30]);
    IOT_SENSITIVE_INFO_MAC("BLE mac :", &data->mn_data[14]);
}
...
static int generate_advertise_data(struct iot_context *ctx, adv_data *data)
{
    ...
    local_name_size = strlen(ctx->devconf.device_onboarding_id) > MAX_LOCAL_NAME_SIZE
                          ? MAX_LOCAL_NAME_SIZE
                          : strlen(ctx->devconf.device_onboarding_id);
