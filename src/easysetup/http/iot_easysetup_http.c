...
const char *post_cgi_cmds[] = {
    IOT_ES_URI_POST_KEYINFO,       IOT_ES_URI_POST_CONFIRMINFO,
    IOT_ES_URI_POST_CONFIRM,       IOT_ES_URI_POST_WIFIPROVISIONINGINFO,
    IOT_ES_URI_POST_SETUPCOMPLETE, IOT_ES_URI_POST_LOGS,
};

const char *get_cgi_cmds[] = {
    IOT_ES_URI_GET_DEVICEINFO,      IOT_ES_URI_GET_WIFISCANINFO, IOT_ES_URI_GET_POST_RESPONSE,
    IOT_ES_URI_GET_LOGS_SYSTEMINFO, IOT_ES_URI_GET_LOGS_DUMP,
};
 ...
 * @param[out]	out_payload		output payload buffer for GET method. caller has full responsibility to free
 * this memory.
 * @return	iot_error_t
...
iot_error_t _iot_easysetup_gen_get_payload(struct iot_context *ctx, int cmd, char **out_payload)
{
    ...
    if ((cur_step != ref_step) && (cur_step < IOT_EASYSETUP_STEP_LOG_SYSTEMINFO)) {
        ...
        } else if ((cur_step == IOT_EASYSETUP_STEP_WIFISCANINFO) &&
 ...
 * @param[in]		in_payload	client updated payload via POST method has given to here. this shouldn't be
 * freed inside of this function.
 * @param[out]		out_payload	output payload for http response. caller has full responsibility to free this
 * memory
 * @return		iot_error_t
 * @retval		IOT_ERROR_NONE		success
 */
...
{
    if (err <= IOT_ERROR_EASYSETUP_400_BASE && err > IOT_ERROR_EASYSETUP_500_BASE) {
        return true;
    } else {
        return false;
    }
...
void http_msg_handler(int cmd, char **buffer, enum cgi_type type, char *data_buf)
{
    ...
    if (type == D2D_POST) {
        ...
        if (!err) {
            ...
            buffer_len = payload_len + strlen(http_status_200) + strlen(http_header) +
                         digit_count_payload(payload_len) + strlen(END_OF_HTTP_HEADER) + 1;
            ...
            snprintf(buf, buffer_len, "%s%s%u%s%s", http_status_200, http_header, payload_len, END_OF_HTTP_HEADER,
                     payload);
    ...
    if (err) {
        ...
        buffer_len =
            strlen(http_header) + digit_count_payload(payload_len) + payload_len + strlen(END_OF_HTTP_HEADER) + 1;
        if (_is_400_error(err)) {
            ...
            snprintf(buf, buffer_len, "%s%s%u%s%s", http_status_400, http_header, payload_len, END_OF_HTTP_HEADER, ptr);
        } else {
            ...
            snprintf(buf, buffer_len, "%s%s%u%s%s", http_status_500, http_header, payload_len, END_OF_HTTP_HEADER, ptr);
        }
    }
    IOT_DEBUG("%s", buf);
    *buffer = buf;
