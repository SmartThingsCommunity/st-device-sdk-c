...
static iot_error_t http_packet_read(PORT_NET_CONTEXT handle, char *rx_buffer, size_t rx_buffer_size,
...
static iot_error_t http_packet_read_remaining(PORT_NET_CONTEXT handle, char *rx_buffer, size_t rx_buffer_size,
...
static int process_accepted_connection(PORT_NET_CONTEXT handle)
{
    ...
    while (1) {
        ...
        if ((err == IOT_ERROR_NONE) && (type == D2D_POST) && payload && (content_len > strlen((char *)payload))) {
            iot_error_t ret;
            ret = http_packet_read_remaining(handle, rx_buffer, sizeof(rx_buffer), received_len,
                                             http_request_header_len + content_len);
