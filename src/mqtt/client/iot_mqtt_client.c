...
{
    int rc = 0;
    port_net_tls_config tls_config = {
        0,
    };
    char port[5] = {0};
...
static int _iot_mqtt_run_write_stream(MQTTClient *client)
{
    ...
    while (written != w_chunk->chunk_size) {
        rc = _iot_mqtt_write_net(client->net_ctx, &w_chunk->chunk_data[written], w_chunk->chunk_size - written);
...
static void _iot_mqtt_process_received_ack(MQTTClient *client, iot_mqtt_packet_chunk_t *chunk)
{
    ...
    if (tmp != NULL) {
        ...
        if (tmp->have_owner) {
            ...
            _iot_mqtt_queue_push(&client->user_event_callback_queue, tmp);
        }
    } else {
...
static int _iot_mqtt_run_read_stream(MQTTClient *client)
{
    ...
    while (read != w_chunk->chunk_size) {
        rc = _iot_mqtt_read_net(client->net_ctx, w_chunk->chunk_data + read, w_chunk->chunk_size - read);
        if (rc < 0) {
            break;
        } else {
            read += rc;
    ...
    if (read == w_chunk->chunk_size) {
        ...
        w_chunk->packet_type =
            (w_chunk->chunk_data[0] & MQTT_FIXED_HEADER_PACKET_TYPE_MASK) >> MQTT_FIXED_HEADER_PACKET_TYPE_OFFSET;
...
static void _iot_mqtt_deliver_publish(MQTTClient *client, iot_mqtt_packet_chunk_t *chunk)
{
    ...
    MQTTDeserialize_publish(&dup, &qos, &msg.retained, &id, &topic_name, (unsigned char **)&msg.payload,
                            (int *)&msg.payloadlen, chunk->chunk_data, chunk->chunk_size);
...
static void _iot_mqtt_notify_publish_failed(MQTTClient *client, iot_mqtt_packet_chunk_t *chunk)
{
    ...
    st_mqtt_publish_result result;

    if (chunk->chunk_state == PACKET_CHUNK_WRITE_FAIL) {
        result = ST_MQTT_PUBLISH_RESULT_FAILED;
    } else if (chunk->chunk_state == PACKET_CHUNK_TIMEOUT) {
        result = ST_MQTT_PUBLISH_RESULT_TIMEOUT;
    } else {
        return;
    }

    if (chunk->publish_cb) {
        chunk->publish_cb(chunk_id, result, chunk->publish_cb_user_data);
    }
}

static void _iot_mqtt_notify_publish_success(MQTTClient *client, iot_mqtt_packet_chunk_t *chunk)
{
    unsigned int chunk_id = chunk->chunk_id;

    if (chunk->publish_cb) {
        chunk->publish_cb(chunk_id, ST_MQTT_PUBLISH_RESULT_SUCCESS, chunk->publish_cb_user_data);
    }
}
...
static void _iot_mqtt_process_user_callback(MQTTClient *client)
{
    ...
    while (1) {
        ...
        switch (w_chunk->chunk_state) {
            ...
            case PACKET_CHUNK_ACKNOWLEDGED:
                if (w_chunk->packet_type == PUBLISH) {
                    _iot_mqtt_notify_publish_success(client, w_chunk);
                }
                break;
            ...
            case EVENT_CHUNK_DISCONNECTED:
                client->user_callback_fp(ST_MQTT_EVENT_DISCONNECTED, &w_chunk->return_code,
                                         client->user_callback_user_data);
                break;
            default:
                break;
        }
    ...
    while (1) {
        ...
        switch (chunk->chunk_state) {
            case PACKET_CHUNK_WRITE_PENDING:
                if (rc < 0) {
                    tmp = _iot_mqtt_queue_pop_by_type_and_id(&client->write_pending_queue, chunk->packet_type,
                                                             chunk->packet_id);
                    if (tmp) {
                        goto exit;
                    }
            ...
            case PACKET_CHUNK_ACK_PENDING:
                if (rc < 0) {
                    tmp = _iot_mqtt_queue_pop_by_type_and_id(&client->ack_pending_queue, chunk->packet_type,
                                                             chunk->packet_id);
                    if (tmp) {
                        goto exit;
                    }
...
int st_mqtt_connect(st_mqtt_client client, st_mqtt_broker_info_t *broker, st_mqtt_connect_data *connect_data)
{
    ...
    c->client_id_hash = 5381;
    if (connect_data->clientid) {
        for (const char *p = connect_data->clientid; *p; p++) {
            c->client_id_hash = ((c->client_id_hash << 5) + c->client_id_hash) ^ (*p);
        }
    }
...
exit:
    if (rc < 0) {
        ...
        iot_os_thread_create(_iot_mqtt_listen_socket, "MQTTSocketListen", MQTT_TASK_STACK_SIZE, (void *)c,
                             MQTT_TASK_PRIORITY, &c->socket_thread);
    }
                                                              ...
                                                              st_mqtt_publish_callback publish_cb, void *usr_data)
{
    ...
    if (msg->qos == st_mqtt_qos1 || msg->qos == st_mqtt_qos2) {
        ...
        unsigned int temp_chunk_id = (c->client_id_hash ^ c->next_packetid);
        pub_packet->chunk_id = (int)(temp_chunk_id & 0x7FFFFFFF);
    }

    MQTTSerialize_publish(pub_packet->chunk_data, chunk_size, 0, msg->qos, msg->retained, pub_packet->packet_id, topic,
    ...
    pub_packet->publish_cb = publish_cb;
    pub_packet->publish_cb_user_data = usr_data;
...
int st_mqtt_publish(st_mqtt_client client, st_mqtt_msg *msg)
{
    ...
    pub_packet = _iot_mqtt_push_publish_packet(c, msg, 1, NULL, NULL);
    if (!pub_packet) {
        rc = E_ST_MQTT_FAILURE;
        goto exit;
    }
    chunk_id = pub_packet->chunk_id;
...
int st_mqtt_publish_async(st_mqtt_client client, st_mqtt_msg *msg, st_mqtt_publish_callback publish_cb, void *usr_data)
{
    MQTTClient *c = client;
    int rc = 0;
    iot_mqtt_packet_chunk_t *pub_packet = NULL;

    pub_packet = _iot_mqtt_push_publish_packet(c, msg, 0, publish_cb, usr_data);
    if (pub_packet == NULL) {
        rc = E_ST_MQTT_FAILURE;
    } else {
        rc = pub_packet->chunk_id;
    }
