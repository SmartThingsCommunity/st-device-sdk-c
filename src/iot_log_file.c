...
static void _iot_log_file_init_header(struct iot_log_file_header_tag *log_file_header)
{
    ...
    log_file_header->sector.num =
        (IOT_LOG_FILE_FLASH_ADDR + IOT_LOG_FILE_FLASH_HEADER_SIZE) / IOT_LOG_FILE_FLASH_SECTOR_SIZE;
...
static iot_log_file_header_state_t _iot_log_file_load_header(struct iot_log_file_header_tag *log_file_header)
{
    ...
    if ((log_file_header->magic_code[0] == 'L') && (log_file_header->magic_code[1] == 'O') &&
        (log_file_header->magic_code[2] == 'G')) {
...
static iot_error_t _iot_log_file_write_sector(unsigned int sector_num, unsigned int offset, void *data_addr,
                                              unsigned int size)
{
    iot_error_t iot_err = IOT_ERROR_NONE;
...
static iot_error_t _iot_log_file_load_old_data(void *buf, unsigned int *file_write_size,
...
static void _iot_log_file_copy_buf(void *iot_file_buf, void *iot_log_buf, unsigned int offset,
                                   unsigned int *file_write_size, unsigned int size)
{
...
static iot_error_t _iot_log_file_update_header(void *buf, struct iot_log_file_header_tag *log_file_header,
...
static iot_error_t _iot_log_file_manager(struct iot_log_file_ctx *ctx)
{
    ...
    _iot_log_file_copy_buf(ctx->file_buf, ctx->log_buf.buf, ctx->file_header.sector.offset, &file_write_size,
                           log_buf_size);
...
static void _iot_log_file_task(void *arg)
{
    ...
    _iot_log_file_enable(IOT_LOG_FILE_TRUE);
    while (1) {
        curr_events = iot_os_eventgroup_wait_bits(log_ctx->events, IOT_LOG_FILE_EVENT_SYNC_REQ_BIT, true, 0xffffffff);
...
iot_log_file_handle_t *iot_log_file_open(size_t *filesize, iot_log_file_type_t file_type)
{
    ...
    switch (file_type) {
        ...
        case FLASH_WITH_RAM: {
            ...
            file_handle->tail_addr =
                _iot_log_file_sector_to_addr(log_file_header.sector.num, log_file_header.sector.offset);
            ...
            *filesize = log_file_header.written_size;
            file_handle->log_size = *filesize;
            IOT_LOG_FILE_DEBUG("file handle start=0x%x cur=0x%x end=0x%x *filesize=0x%x(%d)\n", file_handle->start_addr,
                               file_handle->cur_addr, file_handle->log_size, *filesize, *filesize);
            break;
        }
#endif
        default:
...
iot_error_t iot_log_file_read(iot_log_file_handle_t *file_handle, void *buffer, size_t buf_size, size_t *read_size)
{
