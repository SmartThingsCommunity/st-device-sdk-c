...
/* Make random uuid derived from input key, time and random value */
#define INPUT_RANDOM_VALUE_SIZE	8
iot_error_t iot_get_random_uuid_from_key(struct iot_uuid *uuid, char *key, size_t key_len)
{
	...
	err = iot_bsp_system_get_time_in_sec(&curr_time);
	if (err != IOT_ERROR_NONE) {
		IOT_ERROR("Failed to get current time");
		return IOT_ERROR_UUID_FAIL;
	}

	buf_len = key_len + sizeof(time_t) + INPUT_RANDOM_VALUE_SIZE;
	...
	offset += key_len;
	// Add time info
	memcpy(buf + offset, &curr_time, sizeof(time_t));
	offset += sizeof(time_t);
	// Add random info
	while (offset < buf_len) {
		unsigned int rand_value = iot_bsp_random();
		size_t copy_len = offset + sizeof(unsigned int) > buf_len ? buf_len - offset : sizeof(unsigned int);
		memcpy(buf + offset, (unsigned char*)&rand_value, copy_len);
		offset += copy_len;
	}
