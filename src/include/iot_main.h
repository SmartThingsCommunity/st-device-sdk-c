...
#define DEVICE_PENDING_WORK_SIGNAL (1 << 0)
...
#define DIP_UPDATE_TIMEOUT_MS (5000)
...
enum iot_command_type {
    ...
    IOT_COMMAND_SYSTEM_REBOOT,
    IOT_COMMAND_TYPE_MAX = IOT_COMMAND_SYSTEM_REBOOT, /* MAX : under 32 */
};
...
struct iot_devconf_prov_data {
    ...
    unsigned int
        ownership_validation_type;   /**< @brief onboarding process validation type, JUSTWORKS, BUTTON, PIN, QR */
...
typedef struct _iot_child_device {
    struct _iot_child_device *next;
    struct iot_context *ctx;
    iot_cap_handle_list_t *cap_handle_list;
    char *mqtt_event_topic;

    char deviceId[IOT_REG_UUID_STR_LEN + 1];
    char *mnId;
    char *serial_number;
    struct iot_dip_data dip;
...
struct iot_context {
    ...
    bool registered_msg_requested;                   /**< @brief flag to prevent several register msg */
    ...
    st_device_status device_status; /**< @brief current device status */
    ...
    iot_os_thread work_queue_thread;      /**< @brief iot main work queue thread */
    iot_os_eventgroup *work_queue_signal; /**< @brief work queue thread signal */
    iot_util_queue_t *work_queue;         /**< @brief work task queue */
...
typedef void (*device_work_handler)(struct iot_context *ctx, device_work_param param);

typedef struct _device_work_data {
    device_work_param param;
    device_work_handler handler;
    void *owner_id;
} device_work_data_t;
