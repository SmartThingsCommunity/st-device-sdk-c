...
iot_error_t iot_util_convert_uuid_str(struct iot_uuid *uuid, char *str, size_t max_sz)
{
    ...
    /* dump random uuid */
    for (i = 0; i < 16; i++) {
        wrt = snprintf(str_tmp, sizeof(str_tmp), "%02x", (unsigned char)uuid->id[i]);
        if (wrt != 2) {
            IOT_ERROR("Can't convert id:%02x to str", (unsigned char)uuid->id[i]);
...
iot_error_t iot_util_convert_mac_str(struct iot_mac *mac, char *str, int max_sz)
{
    ...
    for (i = 0; i < 6; i++) {
        wrt = snprintf(str_tmp, sizeof(str_tmp), "%02x", (unsigned char)mac->addr[i]);
        if (wrt != 2) {
            IOT_ERROR("Can't convert mac_addr:%02x to str", (unsigned char)mac->addr[i]);
            return IOT_ERROR_BAD_REQ;
        }
...
void iot_util_print_ssid_secure(const char *func, const int func_line, char *prefix, char *ssid)
{
    ...
    iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s %s", func, func_line, prefix,
                  ssid ? ssid : "NULL");
#else
    if (ssid && strlen(ssid) > 0) {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s %.1s**(%d)", func, func_line, prefix, ssid,
                      strlen(ssid));
    } else {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s", func, func_line, prefix);
    }
#endif
}
...
#if defined(CONFIG_STDK_IOT_CORE_LOG_LEVEL_SENSITIVE_INFO)
    if (mac_address) {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s %02X:%02X:%02X:%02X:%02X:%02X", func,
                      func_line, prefix, mac_address[0], mac_address[1], mac_address[2], mac_address[3], mac_address[4],
                      mac_address[5]);
    } else {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s NULL", func, func_line, prefix);
    }
#else
    if (mac_address) {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s **:**:**:**:%02X:%02X", func, func_line,
                      prefix, mac_address[4], mac_address[5]);
    } else {
        iot_bsp_debug(IOT_DEBUG_LEVEL_INFO, IOT_DEBUG_PREFIX, "%s(%d) > %s NULL", func, func_line, prefix);
    }
#endif
