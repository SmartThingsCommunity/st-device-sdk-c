...
struct iot_cap_handle_list *_iot_cap_create_new_cap_handle(struct iot_context *ctx, const char *component,
                                                           const char *capability, st_cap_init_cb init_cb,
                                                           void *init_usr_data)
{
    ...
    memset(new_list, 0, sizeof(iot_cap_handle_list_t));
                                   ...
                                   st_cap_init_cb init_cb, void *init_usr_data)
{
    ...
    new_list = _iot_cap_create_new_cap_handle(ctx, component, capability, init_cb, init_usr_data);
    if (new_list == NULL) {
        return NULL;
    }
    new_list->next = ctx->cap_handle_list;
    ctx->cap_handle_list = new_list;

    return (IOT_CAP_HANDLE *)new_list->handle;
}
...
{
    struct iot_cap_handle_list *new_list;
    iot_child_device *child_dev_internal = (iot_child_device *)child_dev;

    if (!child_dev || !capability) {
        return NULL;
    }

    new_list = _iot_cap_create_new_cap_handle(child_dev_internal->ctx, component, capability, init_cb, init_usr_data);
    if (new_list == NULL) {
        return NULL;
    }
    new_list->next = child_dev_internal->cap_handle_list;
    child_dev_internal->cap_handle_list = new_list;
    new_list->handle->child_dev = child_dev_internal;
...
int st_cap_send_attr(IOT_EVENT *event[], uint8_t evt_num)
{
    ...
    child_dev = evt_data[0]->ref_cap->child_dev;
    ...
    msg.retained = false;
    if (child_dev) {
        msg.topic = child_dev->mqtt_event_topic;
    } else {
...
STATIC_FUNCTION
iot_error_t _iot_subscribe_child_devices_command(struct iot_context *ctx, JSON_H *child_devices_array)
{
    int i;
    int child_devices_count = JSON_GET_ARRAY_SIZE(child_devices_array);
    JSON_H *each_child = NULL;
    char **subscribe_topic_list = NULL;
    int *qos = NULL;
    iot_error_t ret = IOT_ERROR_NONE;

    if (child_devices_count == 0) {
        return ret;
    }

    subscribe_topic_list = (char **)iot_os_malloc(sizeof(char *) * child_devices_count);
    if (subscribe_topic_list == NULL) {
        IOT_ERROR("Failed to alloc memory");
        ret = IOT_ERROR_MEM_ALLOC;
        goto exit;
    }
    memset(subscribe_topic_list, 0, sizeof(char *) * child_devices_count);
    qos = (int *)iot_os_malloc(sizeof(int) * child_devices_count);
    if (qos == NULL) {
        IOT_ERROR("Failed to alloc memory");
        ret = IOT_ERROR_MEM_ALLOC;
        goto exit;
    ...
    for (i = 0; i < child_devices_count; i++) {
        each_child = JSON_GET_ARRAY_ITEM(child_devices_array, i);
        if (each_child == NULL) {
            IOT_ERROR("Failed to get child device %d", i);
            ret = IOT_ERROR_INVALID_ARGS;
            goto exit;
        }
        subscribe_topic_list[i] = (char *)iot_os_malloc(IOT_TOPIC_SIZE);
        if (subscribe_topic_list[i] == NULL) {
            IOT_ERROR("Failed to alloc memory");
            ret = IOT_ERROR_MEM_ALLOC;
            goto exit;
        }
        snprintf(subscribe_topic_list[i], IOT_TOPIC_SIZE, IOT_SUB_TOPIC_COMMAND, JSON_GET_STRING_VALUE(each_child));
        qos[i] = st_mqtt_qos1;
        IOT_INFO("command topic : %s", subscribe_topic_list[i]);
    }

    ret = st_mqtt_subscribe(ctx->evt_mqttcli, child_devices_count, subscribe_topic_list, qos);
    if (ret) {
...
exit:
    if (subscribe_topic_list) {
        for (i = 0; i < child_devices_count; i++) {
            if (subscribe_topic_list[i]) {
                iot_os_free(subscribe_topic_list[i]);
            }
        }
        iot_os_free(subscribe_topic_list);
    }
...
STATIC_FUNCTION
iot_error_t _iot_notify_child_devices_cloud_connected(struct iot_context *ctx, JSON_H *child_devices_array)
{
    int i;
    int child_devices_count = JSON_GET_ARRAY_SIZE(child_devices_array);
    JSON_H *each_child = NULL;
    iot_child_device *child_dev = NULL;
    iot_error_t ret = IOT_ERROR_NONE;
    iot_noti_data_t notify_data = {.type = IOT_NOTI_TYPE_DEV_CLOUD_CONNECTED};
    ...
    for (i = 0; i < child_devices_count; i++) {
        ...
        child_dev = ctx->child_device_list;
        while (child_dev) {
            if (!strncmp(child_dev->deviceId, JSON_GET_STRING_VALUE(each_child), IOT_REG_UUID_STR_LEN)) {
                if (child_dev->noti_cb) {
                    child_dev->noti_cb(&notify_data, child_dev->noti_usr_data);
                }
                break;
            }
...
iot_error_t _iot_parse_noti_data(struct iot_context *ctx, void *data, iot_noti_data_t *noti_data)
{
    ...
    if (!strncmp(noti_type_string, SERVER_NOTI_TYPE_DEVICE_DELETED, strlen(SERVER_NOTI_TYPE_DEVICE_DELETED))) {
        ...
        IOT_DUMP(IOT_DEBUG_LEVEL_INFO, IOT_DUMP_CAPABILITY_QUOTA_LIMIT_RECEIVED, noti_data->raw.quota.used,
                 noti_data->raw.quota.limit);
    } else if (!strncmp(noti_type_string, SERVER_NOTI_TYPE_PREFERENCE_UPDATED,
                        strlen(SERVER_NOTI_TYPE_PREFERENCE_UPDATED))) {
        ...
        if (item == NULL) {
            ...
            int item_size = JSON_GET_ARRAY_SIZE(item);
            ...
            noti_data->raw.preferences.preferences_num = (size_t)item_size;
            noti_data->raw.preferences.preferences_data = iot_os_malloc(sizeof(iot_preference_data) * item_size);
            ...
            for (int i = 0; i < item_size; i++) {
                ...
                if (!sub_item) {
                    IOT_ERROR("Cannot get %dth item data", i);
                    continue;
                }
                ...
                if (preference_value == NULL) {
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_NULL;
                } else if (!strncmp(JSON_GET_STRING_VALUE(preference_type), "string", 6)) {
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_STRING;
                    ...
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_NUMBER;
                    ...
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_BOOLEAN;
                    ...
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_INTEGER;
                    noti_data->raw.preferences.preferences_data[i].preference_data.integer = preference_value->valueint;
                } else {
                    noti_data->raw.preferences.preferences_data[i].preference_data.type = IOT_CAP_VAL_TYPE_UNKNOWN;
                }
            }
        }
        ...
        err = IOT_ERROR_BAD_REQ;
    } else if (!strncmp(noti_type_string, SERVER_NOTI_TYPE_DEVICE_CREATED, strlen(SERVER_NOTI_TYPE_DEVICE_CREATED))) {
        iot_child_device *child_dev = NULL;
        JSON_H *response = NULL;
        JSON_H *dip_key = NULL;
        JSON_H *meta_data = NULL;

        response = JSON_GET_OBJECT_ITEM(json, "response");
        if (response == NULL) {
            IOT_ERROR("there is no response data in notification");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }

        /* Device Id */
        item = JSON_GET_OBJECT_ITEM(response, "deviceId");
        if (item == NULL) {
            IOT_ERROR("there is no deviceId in response");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }
        child_dev = ctx->child_device_list;
        while (child_dev) {
            if (!strncmp(child_dev->deviceId, JSON_GET_STRING_VALUE(item), IOT_REG_UUID_STR_LEN)) {
                break;
            }
            child_dev = child_dev->next;
        }
        if (!child_dev) {
            child_dev = (iot_child_device *)iot_os_malloc(sizeof(iot_child_device));
            if (child_dev == NULL) {
                IOT_ERROR("Failed to alloc new child device struct");
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            memset(child_dev, 0, sizeof(iot_child_device));
            memcpy(child_dev->deviceId, JSON_GET_STRING_VALUE(item), IOT_REG_UUID_STR_LEN);

            child_dev->ctx = ctx;

            /* mnId, serial number */
            meta_data = JSON_GET_OBJECT_ITEM(response, "metadata");
            if (meta_data == NULL) {
                IOT_ERROR("there is no metadata in response");
                iot_os_free(child_dev);
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }

            item = JSON_GET_OBJECT_ITEM(meta_data, "mnId");
            if (item == NULL) {
                IOT_ERROR("there is no mnId in metadata");
                iot_os_free(child_dev);
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            child_dev->mnId = iot_os_strdup(JSON_GET_STRING_VALUE(item));
            if (child_dev->mnId == NULL) {
                IOT_ERROR("Failed to alloc mnId for child device");
                iot_os_free(child_dev);
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }

            item = JSON_GET_OBJECT_ITEM(meta_data, "serialNumber");
            if (item == NULL) {
                IOT_ERROR("there is no serial in metadata");
                iot_os_free(child_dev->mnId);
                iot_os_free(child_dev);
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            child_dev->serial_number = iot_os_strdup(JSON_GET_STRING_VALUE(item));
            if (child_dev->serial_number == NULL) {
                IOT_ERROR("Failed to alloc serial number for child device");
                iot_os_free(child_dev->mnId);
                iot_os_free(child_dev);
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            child_dev->mqtt_event_topic = (char *)iot_os_malloc(IOT_TOPIC_SIZE);
            if (!child_dev->mqtt_event_topic) {
                IOT_ERROR("failed to malloc for child device event topic");
            ...
            snprintf(child_dev->mqtt_event_topic, IOT_TOPIC_SIZE, IOT_PUB_TOPIC_EVENT, child_dev->deviceId);

            child_dev->next = ctx->child_device_list;
            ctx->child_device_list = child_dev;
        }

        /* dip Id*/
        dip_key = JSON_GET_OBJECT_ITEM(response, "deviceIntegrationProfileKey");
        if (dip_key == NULL) {
            IOT_ERROR("there is no DIP in response");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }
        item = JSON_GET_OBJECT_ITEM(dip_key, "id");
        iot_util_convert_str_uuid(JSON_GET_STRING_VALUE(item), &child_dev->dip.dip_id);
        item = JSON_GET_OBJECT_ITEM(dip_key, "majorVersion");
        if (!item) {
            IOT_ERROR("Can't find majorVersion for dip_key!!");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }
        child_dev->dip.dip_major_version = item->valueint;
        item = JSON_GET_OBJECT_ITEM(dip_key, "minorVersion");
        if (item) {
            child_dev->dip.dip_minor_version = item->valueint;
        }

        noti_data->type = _IOT_NOTI_TYPE_CHILD_DEVICE_REGISTERED;
        noti_data->raw.child_device_registered.child_dev = child_dev;
        noti_data->raw.child_device_registered.mnId = child_dev->mnId;
        noti_data->raw.child_device_registered.serial_number = child_dev->serial_number;
    } else if (!strncmp(noti_type_string, SERVER_NOTI_TYPE_DEVICE_GET, strlen(SERVER_NOTI_TYPE_DEVICE_GET))) {
        iot_child_device *child_dev = NULL;
        JSON_H *child_devices = NULL;
        JSON_H *each_child = NULL;
        JSON_H *dip_key = NULL;

        child_devices = JSON_GET_OBJECT_ITEM(json, "childDevices");
        if (child_devices == NULL) {
            IOT_ERROR("there is no child devices data in notification");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }

        for (int i = 0; i < JSON_GET_ARRAY_SIZE(child_devices); i++) {
            each_child = JSON_GET_ARRAY_ITEM(child_devices, i);

            /* Device Id */
            item = JSON_GET_OBJECT_ITEM(each_child, "id");
            if (item == NULL) {
                IOT_ERROR("there is no deviceId in child info");
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            ...
            if (!child_dev) {
                ...
                /* mnId, serial number */
                item = JSON_GET_OBJECT_ITEM(each_child, "mnId");
                if (item == NULL) {
                    IOT_ERROR("there is no mnId in child info");
                    iot_os_free(child_dev);
                    err = IOT_ERROR_BAD_REQ;
                    goto out_noti_parse;
                }
                ...
                item = JSON_GET_OBJECT_ITEM(each_child, "serialNumber");
                if (item == NULL) {
                    IOT_ERROR("there is no serial in child info");
                    iot_os_free(child_dev->mnId);
                    iot_os_free(child_dev);
                    err = IOT_ERROR_BAD_REQ;
                    goto out_noti_parse;
            ...
            /* dip Id*/
            dip_key = JSON_GET_OBJECT_ITEM(each_child, "deviceIntegrationProfileKey");
            if (dip_key == NULL) {
                IOT_ERROR("there is no DIP in child info");
                err = IOT_ERROR_BAD_REQ;
                goto out_noti_parse;
            }
            item = JSON_GET_OBJECT_ITEM(dip_key, "id");
        ...
        iot_update_child_devices_health(ctx, NULL);
        noti_data->type = _IOT_NOTI_TYPE_CHILD_DEVICE_SYNCED;
    } else if (!strncmp(noti_type_string, SERVER_NOTI_TYPE_CHILD_DEVICE_HEALTH_RESPONSE,
                        strlen(SERVER_NOTI_TYPE_CHILD_DEVICE_HEALTH_RESPONSE))) {
        item = JSON_GET_OBJECT_ITEM(json, "success");
        if (item == NULL) {
            IOT_ERROR("there is success array in secondary health response");
            err = IOT_ERROR_BAD_REQ;
            goto out_noti_parse;
        }
        _iot_subscribe_child_devices_command(ctx, item);
        _iot_notify_child_devices_cloud_connected(ctx, item);
        err = IOT_ERROR_BAD_REQ;
    } else {
...
static iot_error_t _iot_process_cmd(iot_cap_handle_list_t *cap_handle_list, char *component_name, char *capability_name,
                                    char *command_name, iot_cap_cmd_data_t *cmd_data)
{
    ...
    while (command_list != NULL) {
        ...
        if (!strcmp(command_name, command->cmd_type)) {
            command->cmd_cb((IOT_CAP_HANDLE *)handle, cmd_data, command->usr_data);
            break;
        }
    ...
    if (command_list == NULL) {
        IOT_WARN("Not registered cmd set received '%s'", command_name);
        return IOT_ERROR_BAD_REQ;
    }
...
iot_error_t _iot_parse_cmd_data_v2(JSON_H *cmditem, st_command_data *cmd_data)
{
    ...
    if (cap_component == NULL || cap_capability == NULL || cap_command == NULL || cap_args == NULL ||
        command_id == NULL) {
    ...
    if (arr_size > 0) {
        ...
        if (cmd_data->param_list == NULL) {
            IOT_ERROR("Failed to malloc for cmd data param list");
            return IOT_ERROR_MEM_ALLOC;
        }
    ...
    IOT_DEBUG("component:%s, capability:%s command:%s", cmd_data->custom_component_name, cmd_data->custom_cap_name,
...
void iot_cap_commands_cb(struct iot_context *ctx, char *payload)
{
    ...
    int i;
    int arr_size = 0;
    iot_noti_data_t command_noti = {
        .type = IOT_NOTI_TYPE_COMMANDS, .raw.commands.commands_data = NULL, .raw.commands.commands_num = 0};
    ...
    if (!command_noti.raw.commands.commands_data) {
        IOT_ERROR("Failed to malloc command_noti cmd data");
        goto out;
    }
                                ...
                                iot_cap_cmd_data_t *cmd_data)
{
    ...
    *command = iot_os_strdup(cap_command->valuestring);
    ...
    if (subitem != NULL) {
        cmd_data->args_str = (char **)iot_os_malloc(sizeof(char *) * arr_size);
        if (!cmd_data->args_str) {
            IOT_ERROR("Failed to malloc args_str");
            return IOT_ERROR_MEM_ALLOC;
        }
        memset(cmd_data->args_str, 0, sizeof(char *) * arr_size);
        cmd_data->cmd_data = (iot_cap_val_t *)iot_os_malloc(sizeof(iot_cap_val_t) * arr_size);
        if (!cmd_data->cmd_data) {
            IOT_ERROR("Failed to malloc cmd_data");
            iot_os_free(cmd_data->args_str);
            return IOT_ERROR_MEM_ALLOC;
        }
        memset(cmd_data->cmd_data, 0, sizeof(iot_cap_val_t) * arr_size);
        for (i = 0; i < arr_size; i++) {
            if (JSON_IS_BOOL(subitem)) {
                                  ...
                                  int seq_num)
{
    ...
    if (evt_data->evt_value.type == IOT_CAP_VAL_TYPE_BOOLEAN) {
        ...
        if (evt_data->evt_value.str_num == 0) {
            ...
            evt_subarr =
                JSON_CREATE_STRING_ARRAY((const char **)evt_data->evt_value.strings, evt_data->evt_value.str_num);
        }
...
void iot_cap_call_init_cb(iot_cap_handle_list_t *cap_handle_list)
{
    ...
    while (cur_list != NULL) {
        ...
        if (handle && handle->init_cb) {
            IOT_INFO("Call init_cb for %s capability", handle->capability ? handle->capability : "NULL");
