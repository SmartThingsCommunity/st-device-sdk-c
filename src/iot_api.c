    ...
    switch (wifi_mode) {
        ...
        case IOT_WIFI_MODE_STATION:
            ...
            if (wifi_mode == IOT_WIFI_MODE_STATION) {
                memcpy(wifi_conf.ssid, ctx->prov_data.wifi.ssid, strlen(ctx->prov_data.wifi.ssid));
                memcpy(wifi_conf.pass, ctx->prov_data.wifi.password, strlen(ctx->prov_data.wifi.password));
                if (ctx->prov_data.wifi.mac_str[0] != '\0') {
                    memcpy(wifi_conf.bssid, ctx->prov_data.wifi.bssid.addr, IOT_WIFI_MAX_BSSID_LEN);
                }
        ...
        case IOT_WIFI_MODE_SCAN:
            ...
            if (!ctx->scan_result) {
                ctx->scan_result =
                    (iot_wifi_scan_result_t *)iot_os_malloc(IOT_WIFI_MAX_SCAN_RESULT * sizeof(iot_wifi_scan_result_t));
                if (!ctx->scan_result) {
...
iot_error_t iot_state_update(struct iot_context *ctx, iot_state_t new_state, int opt)
{
    ...
    if ((new_state == IOT_STATE_PROV_CONFIRM) && (opt == IOT_STATE_OPT_NEED_INTERACT)) {
        IOT_INFO("Trigger PROV_CONFIRM");
        if (ctx->status_cb && ctx->curr_otm_feature == OVF_BIT_BUTTON) {
            ctx->status_cb(ST_DEVICE_STATUS_ONBOARDING_NEED_CONFIRM, ctx->status_usr_data);
            ctx->device_status = ST_DEVICE_STATUS_ONBOARDING_NEED_CONFIRM;
        }
    }
    ...
    err = iot_command_send(ctx, IOT_COMMAND_STATE_UPDATE, &state_data, sizeof(struct iot_state_data));
...
struct iot_dip_data *_load_dip_info_from_json(JSON_H *dip)
{
    ...
    iot_err = iot_util_convert_str_uuid(JSON_GET_STRING_VALUE(item), &new_dip->dip_id);
    if (iot_err != IOT_ERROR_NONE) {
        iot_os_free(new_dip);
        return NULL;
    }
...
load_out:
    if (iot_err == IOT_ERROR_UNINITIALIZED) {
        if (item && JSON_IS_STRING(item)) {
            IOT_ERROR("[%s] wrong onboarding config value detected: %s", current_name, JSON_GET_STRING_VALUE(item));
        } else {
...
iot_error_t iot_get_time_in_ms(char *buf, size_t buf_len)
{
    ...
    gettimeofday(&tv_now, NULL);
    snprintf(buf, buf_len, "%lld%03ld", (long long)tv_now.tv_sec, (tv_now.tv_usec / 1000));
...
iot_error_t iot_api_read_device_identity(unsigned char *nv_prof, unsigned int nv_prof_len, const char *object,
                                         char **nv_data)
{
    iot_error_t iot_err = IOT_ERROR_NONE;
    JSON_H *root = NULL;
...
load_out:
    if (iot_err == IOT_ERROR_UNINITIALIZED) {
        if (item && JSON_IS_STRING(item)) {
            IOT_ERROR("[%s] wrong nv profile value detected: %s", current_name, JSON_GET_STRING_VALUE(item));
        } else {
...
static iot_error_t _get_dip_from_json(JSON_H *json, struct iot_dip_data *dip)
{
    ...
    iot_err = iot_util_convert_str_uuid(JSON_GET_STRING_VALUE(item), &curr_dip.dip_id);
    if (iot_err != IOT_ERROR_NONE) {
...
static iot_error_t _get_location_from_json(JSON_H *json, struct iot_uuid *uuid)
{
    ...
    iot_err = iot_util_convert_str_uuid(JSON_GET_STRING_VALUE(item), &curr_uuid);
    if (iot_err != IOT_ERROR_NONE) {
        IOT_ERROR("Can't convert str to uuid(%d)", iot_err);
        return iot_err;
    }
...
static iot_error_t _set_dip_to_json(JSON_H *json, struct iot_dip_data *new_dip)
{
    ...
    iot_err = iot_util_convert_uuid_str(&new_dip->dip_id, dip_id_str, sizeof(dip_id_str));
    if (iot_err != IOT_ERROR_NONE) {
...
static iot_error_t _set_location_to_json(JSON_H *json, struct iot_uuid *uuid)
{
    ...
    iot_err = iot_util_convert_uuid_str(uuid, location_id, sizeof(location_id));
    if (iot_err != IOT_ERROR_NONE) {
        IOT_ERROR("Can't convert uuid to str(%d)", iot_err);
        return iot_err;
    }
...
iot_error_t iot_misc_info_store(iot_misc_info_t type, const void *in_data)
{
    ...
    JSON_H *json = NULL;
    iot_error_t iot_err = IOT_ERROR_NONE;
    unsigned char old_hash[IOT_SECURITY_SHA256_LEN] = {
        0,
    };
    unsigned char new_hash[IOT_SECURITY_SHA256_LEN] = {
        0,
    };
    bool hash_chk = false;
    ...
    if (iot_err != IOT_ERROR_NONE) {
        ...
        if (json == NULL) {
            IOT_WARN("old misc_info(%s/%u) parsing failed", old_misc_info, (unsigned int)old_misc_info_len);
            json = JSON_CREATE_OBJECT();
        } else {
            old_misc_avail = true;
        }
    ...
    if (old_misc_avail) {
        iot_err =
            iot_security_sha256((const unsigned char *)old_misc_info, old_misc_info_len, old_hash, sizeof(old_hash));
        if (iot_err != IOT_ERROR_NONE) {
    ...
    if (hash_chk) {
        iot_err = iot_security_sha256((const unsigned char *)new_misc_info, strlen(new_misc_info), new_hash,
                                      sizeof(new_hash));
        if (iot_err != IOT_ERROR_NONE) {
...
iot_error_t iot_update_wifi_info(struct iot_context *ctx)
{
    ...
    JSON_ADD_ITEM_TO_ARRAY(evt_arr,
                           _iot_make_wifi_auth_evt_data(iot_bsp_wifi_get_auth_mode(), ctx->event_sequence_num));
...
iot_error_t iot_update_child_devices_health(struct iot_context *ctx, iot_child_device *target_child_dev)
{
    int ret;
    st_mqtt_msg msg = {0};
    JSON_H *health_status_event = NULL;
    JSON_H *child_devices_health_status = NULL;
    JSON_H *child_device_health = NULL;
    ...
    health_status_event = JSON_CREATE_OBJECT();
    JSON_ADD_STRING_TO_OBJECT(health_status_event, "event", "secondary.devices.health");

    child_devices_health_status = JSON_CREATE_ARRAY();
    if (target_child_dev) {
        child_device_health = JSON_CREATE_OBJECT();
        JSON_ADD_STRING_TO_OBJECT(child_device_health, "deviceId", target_child_dev->deviceId);
        JSON_ADD_STRING_TO_OBJECT(child_device_health, "status", "online");
        JSON_ADD_ITEM_TO_ARRAY(child_devices_health_status, child_device_health);
    } else {
        child_dev = ctx->child_device_list;
        while (child_dev) {
            if (!child_dev->is_online) {
                child_dev = child_dev->next;
                continue;
            }
            child_device_health = JSON_CREATE_OBJECT();
            JSON_ADD_STRING_TO_OBJECT(child_device_health, "deviceId", child_dev->deviceId);
            JSON_ADD_STRING_TO_OBJECT(child_device_health, "status", "online");
            JSON_ADD_ITEM_TO_ARRAY(child_devices_health_status, child_device_health);
            child_dev = child_dev->next;
        }
    }
    JSON_ADD_ITEM_TO_OBJECT(health_status_event, "descendants", child_devices_health_status);

#if defined(STDK_IOT_CORE_SERIALIZE_CBOR)
    iot_serialize_json2cbor(health_status_event, (uint8_t **)&msg.payload, (size_t *)&msg.payloadlen);
#else
    msg.payload = JSON_PRINT(health_status_event);
    if (msg.payload != NULL) {
        msg.payloadlen = strlen(msg.payload);
    }
#endif
    JSON_DELETE(health_status_event);
    if (msg.payload == NULL) {
        IOT_ERROR("Fail to transfer to payload");
        return IOT_ERROR_BAD_REQ;
    }
