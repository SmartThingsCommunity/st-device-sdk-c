                                          ...
                                          const int clear_on_exit, const unsigned int wait_time_ms)
{
    if (wait_time_ms == IOT_OS_WAIT_FOREVER) {
        return xEventGroupWaitBits(eventgroup_handle, (const EventBits_t)bits_to_wait_for, clear_on_exit, false,
                                   portMAX_DELAY);
    }
...
iot_os_timer_handle iot_os_timer_create(iot_os_timer_cb cb, unsigned int expiry_time_ms, void *user_data)
{
    ...
    new_timer_handle->timer =
        xTimerCreate("PortTimer", pdMS_TO_TICKS(expiry_time_ms), pdFALSE, new_timer_handle, _port_timer_cb);
