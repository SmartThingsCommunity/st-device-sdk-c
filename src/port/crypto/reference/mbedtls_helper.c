...
#if MBEDTLS_VERSION_NUMBER > 0x03000000
    ret = mbedtls_ecp_point_write_binary(&mbed_ecp_keypair->MBEDTLS_PRIVATE(grp), &mbed_ecp_keypair->MBEDTLS_PRIVATE(Q),
                                         MBEDTLS_ECP_PF_UNCOMPRESSED, &olen, raw, sizeof(raw));
#else
    ret = mbedtls_ecp_point_write_binary(&mbed_ecp_keypair->grp, &mbed_ecp_keypair->Q, MBEDTLS_ECP_PF_UNCOMPRESSED,
                                         &olen, raw, sizeof(raw));
#endif
    if (ret) {
        IOT_ERROR("mbedtls_ecp_point_write_binary = -0x%04X", -ret);
...
iot_error_t mbedtls_helper_gen_secp256r1_keypair(iot_security_buffer_t *seckey_buf, iot_security_buffer_t *pubkey_buf)
{
    ...
    ret = mbedtls_ctr_drbg_seed(&mbed_ctr_drbg, mbedtls_entropy_func, &mbed_entropy, (const unsigned char *)pers,
                                strlen(pers));
    if (ret) {
...
#if MBEDTLS_VERSION_NUMBER > 0x03000000
    ret = mbedtls_pk_parse_key(&mbed_pk_context, (const unsigned char *)pk_params->seckey.p, pk_params->seckey.len + 1,
                               NULL, 0, NULL, NULL);
#else
    ret = mbedtls_pk_parse_key(&mbed_pk_context, (const unsigned char *)pk_params->seckey.p, pk_params->seckey.len + 1,
                               NULL, 0);
...
#if MBEDTLS_VERSION_NUMBER > 0x03000000
    ret = mbedtls_pk_sign(&mbed_pk_context, mbed_md_type, input_buf->p, input_buf->len, sig_buf->p, 256, &sig_buf->len,
                          NULL, NULL);
#else
    ret = mbedtls_pk_sign(&mbed_pk_context, mbed_md_type, input_buf->p, input_buf->len, sig_buf->p, &sig_buf->len, NULL,
                          NULL);
#endif
    if (ret) {
...
#if MBEDTLS_VERSION_NUMBER > 0x03000000
    ret = mbedtls_pk_sign(&mbed_pk_context, mbed_md_type, input_buf->p, input_buf->len, sig_buf->p, 1024, &sig_buf->len,
                          NULL, NULL);
#else
    ret = mbedtls_pk_sign(&mbed_pk_context, mbed_md_type, input_buf->p, input_buf->len, sig_buf->p, &sig_buf->len, NULL,
                          NULL);
...
static iot_error_t _cipher_aes_check_info(iot_security_cipher_params_t *cipher_params,
                                                     ...
                                                     iot_security_buffer_t *output_buf)
{
    ...
    ret = mbedtls_ecdh_compute_shared(&mbed_ecdh.MBEDTLS_PRIVATE(grp), &mbed_ecdh.MBEDTLS_PRIVATE(z),
                                      &mbed_ecdh.MBEDTLS_PRIVATE(Qp), &mbed_ecdh.MBEDTLS_PRIVATE(d),
    ...
    ret = mbedtls_ecdh_compute_shared(&mbed_ecdh.grp, &mbed_ecdh.z, &mbed_ecdh.Qp, &mbed_ecdh.d,
                                      mbedtls_ctr_drbg_random, &mbed_ctr_drbg);
    if (ret) {
        IOT_ERROR("mbedtls_ecdh_compute_shared = -0x%04X", -ret);
        err = IOT_ERROR_SECURITY_ECDH_LIBRARY;
