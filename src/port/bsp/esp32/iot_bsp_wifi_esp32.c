...
static void esp_wifi_event_post_to_user(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    switch (id) {
        ...
        case WIFI_EVENT_AP_STACONNECTED: {
            ...
            IOT_SENSITIVE_INFO_MAC("station:", event->mac);
            if (wifi_event_cb) {
...
static esp_err_t esp_event_loop_init_wrap(void *ctx)
{
    ...
    err = esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &esp_wifi_event_post_to_user, NULL, NULL);
    if (err != ESP_OK) {
    ...
    err = esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID, &esp_ip_event_post_to_user, NULL, NULL);
    if (err != ESP_OK) {
...
iot_error_t _set_wifi_station_mode()
{
    ...
    if (mode == WIFI_MODE_STA) {
        ...
        uxBits =
            xEventGroupWaitBits(wifi_event_group, WIFI_AP_STOP_BIT, true, false, pdMS_TO_TICKS(IOT_WIFI_CMD_TIMEOUT));
    ...
    if (need_to_set_mode) {
        ...
        uxBits =
            xEventGroupWaitBits(wifi_event_group, WIFI_STA_START_BIT, true, false, pdMS_TO_TICKS(IOT_WIFI_CMD_TIMEOUT));
    ...
    switch (conf->mode) {
        ...
        case IOT_WIFI_MODE_STATION:
            ...
            if (wifi_current_state == WIFI_EVENT_STA_CONNECTED) {
                ...
                uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_STA_DISCONNECT_BIT, true, false,
                                             pdMS_TO_TICKS(IOT_WIFI_CMD_TIMEOUT));
            ...
            if (str_len) {
                memcpy(wifi_config.sta.ssid, conf->ssid,
                       (str_len > IOT_WIFI_MAX_SSID_LEN) ? IOT_WIFI_MAX_SSID_LEN : str_len);
            }

            str_len = strlen(conf->pass);
            if (str_len) {
                memcpy(wifi_config.sta.password, conf->pass,
                       (str_len > IOT_WIFI_MAX_PASS_LEN) ? IOT_WIFI_MAX_PASS_LEN : str_len);
            }
            ...
            if (str_len) {
                ...
                IOT_DEBUG("target mac=%2X:%2X:%2X:%2X:%2X:%2X", wifi_config.sta.bssid[0], wifi_config.sta.bssid[1],
                          wifi_config.sta.bssid[2], wifi_config.sta.bssid[3], wifi_config.sta.bssid[4],
            ...
            IOT_SENSITIVE_INFO_SSID("connect to ap SSID:", (char *)wifi_config.sta.ssid);
            uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_STA_CONNECT_BIT | WIFI_STA_DISCONNECT_BIT, true, false,
                                         pdMS_TO_TICKS(IOT_WIFI_CMD_TIMEOUT));
        ...
        case IOT_WIFI_MODE_SOFTAP:
            ...
            str_len = strlen(conf->ssid);
            memcpy(wifi_config.ap.ssid, conf->ssid,
                   (str_len > IOT_WIFI_MAX_SSID_LEN) ? IOT_WIFI_MAX_SSID_LEN : str_len);

            str_len = strlen(conf->pass);
            memcpy(wifi_config.ap.password, conf->pass,
                   (str_len > IOT_WIFI_MAX_PASS_LEN) ? IOT_WIFI_MAX_PASS_LEN : str_len);
            ...
            IOT_DEBUG("wifi_init_softap finished.SSID:%s password:%s", wifi_config.ap.ssid, wifi_config.ap.password);

            uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_AP_START_BIT, true, false,
                                         pdMS_TO_TICKS(IOT_WIFI_CMD_TIMEOUT));
        ...
        if (ap_num > 0) {
            ...
            if (esp_wifi_scan_get_ap_records(&ap_num, ap_list) == ESP_OK) {
                for (i = 0; i < ap_num; i++) {
                    ...
                    IOT_DEBUG(
                        "scan result ssid=%s, mac=%02X:%02X:%02X:%02X:%02X:%02X, rssi=%d, freq=%d, authmode=%d chan=%d",
                        scan_result[i].ssid, scan_result[i].bssid[0], scan_result[i].bssid[1], scan_result[i].bssid[2],
