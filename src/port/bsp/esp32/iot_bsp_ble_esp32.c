...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT: {
            ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, conn_id %u, trans_id %lu, handle %u need resp %u is_prep %u",
                     param->write.conn_id, param->write.trans_id, param->write.handle, param->write.need_rsp,
                     param->write.is_prep);
        ...
        case ESP_GATTS_CONNECT_EVT: {
            ...
            ESP_LOGI(GATTS_TAG,
                     "ESP_GATTS_CONNECT_EVT, conn_id %u, remote %02x:%02x:%02x:%02x:%02x:%02x:", param->connect.conn_id,
        ...
        case ESP_GATTS_CONF_EVT:
            ESP_LOGI(GATTS_TAG, "ESP_GATTS_CONF_EVT, status 0x%02X attr_handle %u", param->conf.status,
                     param->conf.handle);
