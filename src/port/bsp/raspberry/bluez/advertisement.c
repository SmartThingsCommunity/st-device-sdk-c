...
int get_bt_dev_address(uint8_t mac_address[6])
{
    ...
    ret = gdbus_method_call_sync((char *)BLUEZ_SERVICE_NAME, (char *)HCI_OBJECT_PATH, (char *)DBUS_PROP_IFACE,
                                 (char *)"Get", g_variant_new("(ss)", ADAPTER_IFACE, "Address"), &reply);
    if (ret) {
    ...
    sscanf(addr, "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx", &mac_address[0], &mac_address[1], &mac_address[2],
