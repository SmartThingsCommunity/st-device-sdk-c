...
static void gatt_build_service(GVariantBuilder *managedBuilder)
{
    ...
    g_variant_builder_add(inner_builder, "{sv}", "UUID", g_variant_new_string(SERVICE_UUID));
    g_variant_builder_add(inner_builder, "{sv}", "Primary", g_variant_new_boolean(TRUE));
...
static void gatt_build_char(GVariantBuilder *managedBuilder)
{
    ...
    g_variant_builder_add(inner_builder, "{sv}", "Service", g_variant_new("o", SERVICE_PATH));
    g_variant_builder_add(inner_builder, "{sv}", "UUID", g_variant_new_string(CHAR_UUID));
...
static void gatt_build_desc(GVariantBuilder *managedBuilder)
{
    ...
    g_variant_builder_add(inner_builder, "{sv}", "Characteristic", g_variant_new("o", CHAR_PATH));
    g_variant_builder_add(inner_builder, "{sv}", "UUID", g_variant_new_string(DESC_UUID));
...
static void handle_app_method_call(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                   ...
                                   GDBusMethodInvocation *invocation, gpointer user_data)
{
    if (g_strcmp0(method_name, "GetManagedObjects") == 0) {
        ...
        g_dbus_method_invocation_return_dbus_error(invocation, "org.gtk.GDBus.NotOnUnix",
                                                   "Your OS does not support file descriptor passing");
    }
}

/* Handle Service */
static void handle_service_method_call(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                       const gchar *interface_name, const gchar *method_name, GVariant *parameters,
                                       GDBusMethodInvocation *invocation, gpointer user_data)
{
...
static GVariant *handle_service_get_property(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
...
static gboolean handle_service_set_property(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                            const gchar *interface_name, const gchar *property_name, GVariant *value,
                                            GError **error, gpointer user_data)
{
    if (g_strcmp0(property_name, "UUID") == 0) {
        IOT_DEBUG("Property Name: %s", property_name);
        g_set_error(error, g_quark_from_string("SetPropertyFailed"), 1, "Unable to set Property");
    } else if (g_strcmp0(property_name, "Primary") == 0) {
        IOT_DEBUG("Property Name: %s", property_name);
        g_set_error(error, g_quark_from_string("SetPropertyFailed"), 1, "Unable to set Property");
    } else {
...
static void handle_char_method_call(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                    const gchar *interface_name, const gchar *method_name, GVariant *parameters,
                                    GDBusMethodInvocation *invocation, gpointer user_data)
{
    if (g_strcmp0(method_name, "ReadValue") == 0) {
        IOT_DEBUG("Method: ReadValue");
        g_dbus_method_invocation_return_dbus_error(invocation, "org.gtk.GDBus.NotSupported", "Read Not Supported");
        ...
        IOT_DEBUG("Method: None");
        g_dbus_method_invocation_return_dbus_error(invocation, "org.gtk.GDBus.NotSupported", "Method Not Supported");
    }
}

static GVariant *handle_char_get_property(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                          const gchar *interface_name, const gchar *property_name, GError **error,
                                          gpointer user_data)
{
    GVariant *ret = NULL;
...
static gboolean handle_char_set_property(GDBusConnection *connection, const gchar *sender, const gchar *object_path,
                                         const gchar *interface_name, const gchar *property_name, GVariant *value,
                                         GError **error, gpointer user_data)
{
    if (g_strcmp0(property_name, "UUID") == 0) {
...
/* for now */
static const GDBusInterfaceVTable app_vtable = {handle_app_method_call, NULL, NULL};

static const GDBusInterfaceVTable service_vtable = {handle_service_method_call, handle_service_get_property,
                                                    handle_service_set_property};

static const GDBusInterfaceVTable char_vtable = {handle_char_method_call, handle_char_get_property,
...
void send_data_over_gatt(uint8_t *data, uint32_t len)
{
    ...
    g_variant_builder_add(outer_builder, "{sv}", "Value", update_value);

    IOT_DEBUG("PATH: %s", app_service.char0->path);
    ret = g_dbus_connection_emit_signal(
        get_gdbus_connection(), NULL, app_service.char0->path, "org.freedesktop.DBus.Properties", "PropertiesChanged",
        g_variant_new("(sa{sv}as)", "org.bluez.GattCharacteristic1", outer_builder, invalidated_builder), &error);

    if (!ret) {
        IOT_ERROR("D-Bus API");
        if (error != NULL) {
            IOT_ERROR(
                "D-Bus API failure: errCode[%x], \
						message[%s]",
...
void start_gatt_service(void)
{
    ...
    app_service.char0->registration_id =
        gdbus_register_object(app_service.char0->path, app_service.char0->xml, app_service.char0->vtable);
    // app_service.char0->desc0->registration_id = gdbus_register_object(app_service.char0->desc0->path,
    // app_service.char0->desc0->xml, app_service.char0->desc0->vtable);
}
...
void register_gatt_application(void)
{
    ...
    gdbus_method_call_async((char *)BLUEZ_SERVICE_NAME, (char *)adapter_path, (char *)GATT_MNGR_INTERFACE,
                            ...
                            bluez_gdbus_call_async_cb, (gpointer) "RegisterApplication");
