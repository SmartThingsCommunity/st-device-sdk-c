...
static void bluez_signal_device_changed(GDBusConnection *conn, const gchar *sender, const gchar *path,
                                        const gchar *interface, const gchar *signal, GVariant *params, void *userdata)
{
    (void)conn;
    (void)sender;
    (void)path;
        ...
        if (!g_strcmp0(key, "Connected")) {
            ...
            if (is_connected && strcmp(connected_device, path)) {
                if (g_variant_get_boolean(value)) {
                    ret = gdbus_method_call_sync((char *)BLUEZ_SERVICE_NAME, (char *)path, (char *)BLUEZ_DEVICE_IFACE,
                                                 (char *)"Disconnect", NULL, &reply);
                    if (ret) {
...
static void bluez_signal_device_added(GDBusConnection *conn, const gchar *sender, const gchar *path,
                                      const gchar *interface, const gchar *signal, GVariant *params, void *userdata)
{
    (void)conn;
    (void)sender;
        ...
        while (g_variant_iter_next(interfaces_and_properties, "{&sa{sv}}", &iface, &properties)) {
            if (!g_strcmp0(iface, BLUEZ_DEVICE_IFACE)) {
                while (g_variant_iter_next(properties, "{&sv}", &key, &value)) {
                    if (!g_strcmp0(key, "Connected")) {
                        if (!g_variant_is_of_type(value, G_VARIANT_TYPE_BOOLEAN)) {
                            g_print("Invalid argument type for %s: %s != %s", key, g_variant_get_type_string(value),
                                    "b");
                            return;
                        }
                        if (is_connected && strcmp(connected_device, object)) {
                            if (g_variant_get_boolean(value)) {
                                ret = gdbus_method_call_sync((char *)BLUEZ_SERVICE_NAME, (char *)object,
                                                             (char *)BLUEZ_DEVICE_IFACE, (char *)"Disconnect", NULL,
                                                             &reply);
                                if (ret) {
                                    g_print("error while sending get interface method call %d\n", ret);
                                }
...
void init_bluez_device(void)
{
    ...
    g_dbus_connection_signal_subscribe(system_gconn, BLUEZ_SERVICE_NAME, DBUS_PROP_IFACE, "PropertiesChanged", NULL,
                                       BLUEZ_DEVICE_IFACE, G_DBUS_SIGNAL_FLAGS_NONE, bluez_signal_device_changed, NULL,
                                       NULL);

    g_dbus_connection_signal_subscribe(system_gconn, BLUEZ_SERVICE_NAME, DBUS_OM_IFACE, "InterfacesAdded", NULL, NULL,
                                       G_DBUS_SIGNAL_FLAGS_NONE, bluez_signal_device_added, NULL, NULL);
}
...
void disconnect_bluez_device()
{
    ...
    if (is_connected) {
        ret = gdbus_method_call_sync((char *)BLUEZ_SERVICE_NAME, (char *)connected_device, (char *)BLUEZ_DEVICE_IFACE,
                                     (char *)"Disconnect", NULL, &reply);
        if (ret) {
