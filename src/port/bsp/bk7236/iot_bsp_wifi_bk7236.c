...
static bk_err_t bk_ip_event_post_to_user(void *arg, event_module_t event_module, int event_id, void *event_data)
{
...
iot_error_t iot_bsp_wifi_set_mode(iot_wifi_conf *conf)
{
    ...
    switch (conf->mode) {
        ...
        case IOT_WIFI_MODE_STATION:
            ...
            uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_STA_CONNECT_BIT, true, false, IOT_WIFI_CMD_TIMEOUT);
        ...
        case IOT_WIFI_MODE_SOFTAP:
            ...
            IOT_DEBUG("wifi_init_softap finished.SSID:%s password:%s", ap_config.ssid, ap_config.password);
...
uint16_t iot_bsp_wifi_get_scan_result(iot_wifi_scan_result_t *scan_result)
{
    ...
    uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_SCAN_DONE_BIT, true, false, IOT_WIFI_CMD_TIMEOUT);
