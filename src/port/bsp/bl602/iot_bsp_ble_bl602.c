...
static uint8_t ble_onboarding_char_uuid[16] = {0x09, 0x0E, 0xE6, 0x80, 0x02, 0x30, 0xC7, 0xA4,
                                               0x8E, 0x4F, 0x2D, 0xAE, 0x0E, 0x0F, 0x94, 0xBE};
#define BLE_ONBOARDING_CHAR_UUID \
    BT_UUID_DECLARE_128(BT_UUID_128_ENCODE(0xBE940F0E, 0xAE2D, 0x4F8E, 0xA4C7, 0x300280E60E09))
...
static int ble_blf_recv(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf, u16_t len, u16_t offset,
                        u8_t flags);
...
static struct bt_gatt_attr blattrs[] = {
    ...
    BT_GATT_CHARACTERISTIC(BLE_ONBOARDING_CHAR_UUID, BT_GATT_CHRC_INDICATE | BT_GATT_CHRC_WRITE | BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, NULL, ble_blf_recv, NULL),
...
static void bt_gatt_indicate_cb(struct bt_conn *conn, const struct bt_gatt_attr *attr, u8_t err)
{
    // add
...
iot_error_t iot_bsp_ble_init(iot_ble_cbs_t *ble_cbs)
{
    ...
    if (g_ble_status == BL_BLE_STATUS_INIT) {
        IOT_INFO("BL BLE already initialised");
...
void iot_bsp_ble_deinit(void)
{
    ...
    if (bt_le_adv_stop()) {
        IOT_ERROR("Failed to stop advertising");
        return;
    }
    if (bt_gatt_service_unregister(&ble_bl_server)) {
        IOT_ERROR("Failed to unregister gatt service");
        return;
    }
    if (bt_disable()) {
        IOT_ERROR("Failed to disable BLE");
        return;
    }
