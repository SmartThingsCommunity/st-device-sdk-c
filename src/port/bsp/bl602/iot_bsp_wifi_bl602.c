    ...
    switch (event->code) {
        ...
        case CODE_WIFI_ON_AP_STA_ADD: { /*
            ...
            IOT_INFO("station: %02x:%02x:%02x:%02x:%02x:%02x join\r\n", sta_info.sta_mac[0], sta_info.sta_mac[1],
                     sta_info.sta_mac[2], sta_info.sta_mac[3], sta_info.sta_mac[4], sta_info.sta_mac[5]);
            if (wifi_event_cb) {
...
iot_error_t iot_bsp_wifi_init()
{
    ...
    uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_INIT_BIT, true, false, IOT_WIFI_CMD_TIMEOUT_BL602);
...
{
    return ((status)&WIFI_STATE_CONNECTING || (status)&WIFI_STATE_CONNECTED_IP_GETTING ||
            (status)&WIFI_STATE_CONNECTED_IP_GOT);
}
    ...
    switch (conf->mode) {
        case IOT_WIFI_MODE_OFF:
            ...
            if (is_ap_started(state)) {
                ...
                uxBits =
                    xEventGroupWaitBits(wifi_event_group, WIFI_AP_STOP_BIT, true, false, IOT_WIFI_CMD_TIMEOUT_BL602);
        ...
        case IOT_WIFI_MODE_SCAN:
            ...
            if (s_wifi_connect_timeout == true) {
                ...
                IOT_ERROR("clean bit: WIFI_STA_CONNECT_BIT");
                uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_STA_DISCONNECT_BIT | WIFI_STA_CONNECT_BIT, true,
                                             false, IOT_WIFI_CMD_TIMEOUT_BL602);
        ...
        case IOT_WIFI_MODE_STATION:
            ...
            if (wifi_mgmr_sta_connect_mid(wifi_interface, ssid, password, NULL, mac, 0, 0, 1,
                                          WIFI_CONNECT_PMF_CAPABLE) == -1) {
                IOT_ERROR("Failed to connect");
                return IOT_ERROR_CONN_OPERATE_FAIL;
            }
            uxBits =
                xEventGroupWaitBits(wifi_event_group, WIFI_STA_CONNECT_BIT, true, false, IOT_WIFI_CMD_TIMEOUT_BL602);
        ...
        case IOT_WIFI_MODE_SOFTAP:
            ...
            wifi_mgmr_ap_start_atcmd(wifi_interface, ap_ssid, ap_hidden_ssid, ap_password, ap_channel,
                                     ap_max_connection);
            IOT_INFO("wifi_init_softap finished.SSID:%s password:%s", ap_ssid, ap_password);

            uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_AP_START_BIT, true, false, IOT_WIFI_CMD_TIMEOUT_BL602);
...
uint16_t iot_bsp_wifi_get_scan_result(iot_wifi_scan_result_t *scan_result)
{
    ...
    uxBits = xEventGroupWaitBits(wifi_event_group, WIFI_SCAN_DONE_BIT, true, false, IOT_WIFI_CMD_TIMEOUT_BL602);
