...
#define PRINT_ST_INFO_LOG(TAG, M, ...)                                                                           \
    do {                                                                                                         \
        __blog_printf(INFO_LOG_COLOR_HEAD "I (%u) %s : %s" INFO_LOG_COLOR_END "\r\n",                            \
                      (xPortIsInsideInterrupt()) ? (xTaskGetTickCountFromISR()) : (xTaskGetTickCount()), TAG, M, \
                      ##__VA_ARGS__);                                                                            \
    } while (0 == 1)

#define PRINT_ST_WARN_LOG(TAG, M, ...)                                                                           \
    do {                                                                                                         \
        __blog_printf(WARN_LOG_COLOR_HEAD "W (%u) %s : %s" WARN_LOG_COLOR_END "\r\n",                            \
                      (xPortIsInsideInterrupt()) ? (xTaskGetTickCountFromISR()) : (xTaskGetTickCount()), TAG, M, \
                      ##__VA_ARGS__);                                                                            \
    } while (0 == 1)

#define PRINT_ST_ERROR_LOG(TAG, M, ...)                                                                          \
    do {                                                                                                         \
        __blog_printf(ERROR_LOG_COLOR_HEAD "E (%u) %s : %s" ERROR_LOG_COLOR_END "\r\n",                          \
                      (xPortIsInsideInterrupt()) ? (xTaskGetTickCountFromISR()) : (xTaskGetTickCount()), TAG, M, \
                      ##__VA_ARGS__);                                                                            \
    } while (0 == 1)

#define PRINT_ST_DEBUG_LOG(TAG, M, ...)                                                                          \
    do {                                                                                                         \
        __blog_printf(DEBUG_LOG_COLOR_HEAD "D (%u) %s : %s" DEBUG_LOG_COLOR_END "\r\n",                          \
                      (xPortIsInsideInterrupt()) ? (xTaskGetTickCountFromISR()) : (xTaskGetTickCount()), TAG, M, \
                      ##__VA_ARGS__);                                                                            \
    } while (0 == 1)
...
void iot_bsp_debug_check_heap(const char *tag, const char *func, const int line, const char *fmt, ...)
{
    ...
    if (count == 0) {
        iot_bsp_debug(IOT_DEBUG_LEVEL_WARN, tag, "%s(%d) > [MEMCHK][%d] Heap total size : %d", func, line, count,
                      _iot_bsp_debug_get_maximum_heap_size());
    }

    iot_bsp_debug(IOT_DEBUG_LEVEL_WARN, tag, "%s(%d) > [MEMCHK][%d][%s] CU:%d, CR:%d, PU:%d, PR:%d", func, line,
                  ++count, buf, _iot_bsp_debug_get_maximum_heap_size() - _iot_bsp_debug_get_free_heap_size(),
