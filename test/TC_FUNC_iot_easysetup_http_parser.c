...
static char *gen_post_method_payload(char *uri, char *body)
{
    ...
    char *client_http_header_1 =
        "User-Agent: Android/Oneapp SDKClient\r\n"
        "Content-Type: application/json; charset=UTF-8\r\n";
    char *client_http_header_2 =
        "Host: 192.168.4.1:8888\r\n"
        "Connection: Keep-Alive\r\n"
        "Accept-Encoding: gzip\r\n";
    int len = 0;
    ...
    len = snprintf(buffer, sizeof(buffer), "POST %s HTTP/1.1\r\n%sContent-Length: %zu\r\n%s\r\n%s", uri,
                   client_http_header_1, strlen(body), client_http_header_2, body);
    assert_true(len > 0);
...
void TC_es_msg_parser_VALID_POST_METHOD(void **state)
{
    ...
    struct cgi_method_test_map map[] = {
        ...
        {IOT_ES_URI_POST_WIFIPROVISIONINGINFO, IOT_EASYSETUP_STEP_WIFIPROVIONINGINFO, D2D_POST,
         "WiFiProvisioning Body"},
...
void TC_es_msg_parser_INVALID_POST_METHOD(void **state)
{
    ...
    // Given: no body part
    char no_body_message[] =
        "POST /confirminfo HTTP/1.1\r\n"
        "User-Agent: Android/Oneapp SDKClient\r\n"
        "Content-Length: 12\r\n"
        "Host: 192.168.4.1:8888\r\n"
        "Connection: Keep-Alive\r\n"
    ...
    type = -1;
    // When
    err = es_msg_parser(no_header_single_body, strlen(no_header_single_body), &parsed_content, &command, &type,
                        &contents_len);
    // Then
    assert_int_not_equal(err, IOT_ERROR_NONE);

    // Given: incomplete header - no value
    char incomplete_header_msg[] =
        "POST /confirm HTTP/1.1\r\n"
        "Content-Length: \r\n"
        "\r\n"
        "test_content";
    ...
    type = -1;
    // When
    err = es_msg_parser(incomplete_header_msg, strlen(incomplete_header_msg), &parsed_content, &command, &type,
                        &contents_len);
    // Then
    assert_int_not_equal(err, IOT_ERROR_NONE);

    // Given: missing content length header
    char missing_content_length_header[] =
        "POST /confirminfo HTTP/1.1\r\n"
    ...
    type = -1;
    // When
    err = es_msg_parser(missing_content_length_header, strlen(missing_content_length_header), &parsed_content, &command,
                        &type, &contents_len);
    // Then
    assert_int_not_equal(err, IOT_ERROR_NONE);
}
